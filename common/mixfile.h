...
template <class T, class TCRC = CRCEngine> class MixFileClass : public _Node<MixFileClass<T>>
{
public:
...
#pragma pack(pop)
    ...
    */
    ...
    static _List<MixFileClass<T, TCRC>> MixList;
};
...
*/
template <class T, class TCRC> _List<MixFileClass<T, TCRC>> MixFileClass<T, TCRC>::MixList;
...
bool MixFileClass<T, TCRC>::Offset(char const* filename, void** realptr, MixFileClass** mixfile, int* offset, int* size)
{
    ...
    */
    ...
    int32_t crc = Calculate_CRC<TCRC>(toupper(filename_upper), int(strlen(filename_upper)));
