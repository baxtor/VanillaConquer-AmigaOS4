    ...
    if (shapefile != NULL && shapenum != -1) {
        ...
        */
        if (flags & SHAPE_CENTER) {
            if (x - width / 2 >= WindowList[window][WINDOWWIDTH])
                return;
            if (y - width / 2 >= WindowList[window][WINDOWHEIGHT])
                return;
            if (x + width / 2 < 0)
                return;
            if (y + height / 2 < 0)
            ...
            if (x >= WindowList[window][WINDOWWIDTH])
                return;
            if (y >= WindowList[window][WINDOWHEIGHT])
                return;
            if (x + width < 0)
                return;
            if (y + height < 0)
                ...
                if ((flags & (SHAPE_GHOST | SHAPE_FADING)) == (SHAPE_GHOST | SHAPE_FADING)) {
                    ...
                    if (flags & SHAPE_FADING) {
                        Buffer_Frame_To_Page(
                            x, y, width, height, buffer, draw_window, flags | SHAPE_TRANS, fadingdata, 1, predoffset);
                    } else {
                        if (flags & SHAPE_PREDATOR) {
                            Buffer_Frame_To_Page(
                                x, y, width, height, buffer, draw_window, flags | SHAPE_TRANS, predoffset);
                        } else {
                            Buffer_Frame_To_Page(
                                x, y, width, height, buffer, draw_window, flags | SHAPE_TRANS, ghostdata, predoffset);
                        }
                    }
                }
                draw_window.Unlock();
...
static void Do_Record_Playback(void)
{
    ...
    */
    if (Session.Record) {
        ...
        */
        Session.RecordFile.Write(&Map.DesiredTacticalCoord, sizeof(Map.DesiredTacticalCoord));
