...
void GameOptionsClass::Process(void)
{
    ...
    */
    ...
    for (int index = 0; index < sizeof(_constants) / sizeof(_constants[0]); index++) {
        ...
        TextButtonClass* g = new TextButtonClass(_constants[index].ID, text, TPF_6PT_GRAD | TPF_NOSHADOW, 0, y);
    ...
    */
    ...
    if (GameToPlay == GAME_NORMAL) {
        ...
        buttonsel[BUTTON_RESTATE - 1]->X =
            OptionX + OptionWidth - (buttonsel[BUTTON_RESTATE - 1]->Width + (5 * resfactor));
    }
    ...
    */
    (new ControlClass(BUTTON_RESUME,
                      0,
                      0,
                      SeenBuff.Get_Width(),
                      SeenBuff.Get_Height(),
