 ...
 *	NET_QUERY_JOIN * Name:					name of player wanting to join								  * PlayerInfo:
 *player's requested house & color								  * NET_CONFIRM_JOIN
 ** PlayerInfo:			approves player's house & color								  * NET_REJECT_JOIN
 ** (no other data)																			  * NET_GAME_OPTIONS
...
bool Process_Global_Packet(GlobalPacketType* packet, IPXAddressClass* address)
{
    ...
    */
    if (packet->Command == NET_QUERY_GAME && NetStealth == 0) {
        ...
        */
        if (packet->Command == NET_QUERY_PLAYER && !strcmp(packet->Name, MPlayerGameName)
...
static int Net_Join_Dialog(void)
{
    ...
    TextButtonClass joinbtn(BUTTON_JOIN,
                            TXT_JOIN,
                            TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
#ifdef FRENCH
                            d_join_x,
    ...
    TextButtonClass newbtn(BUTTON_NEW,
                           TXT_NEW,
                           TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                           d_new_x,
                           d_new_y,
    ...
    TextButtonClass sendbtn(BUTTON_SEND,
                            TXT_SEND_MESSAGE,
                            TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                            //#if (GERMAN | FRENCH)
                            //		d_send_x, d_send_y);
    ...
    */
    ...
    Fancy_Text_Print("", 0, 0, CC_GREEN, TBLACK, TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW);

    Messages.Init(d_message_x + 2, d_message_y + 2, 4, MAX_MESSAGE_LENGTH, d_txt6_h);
    ...
    while (process) {
        ...
        if (display) {
            ...
            */
            ...
            if (display >= REDRAW_MESSAGE) {
                Draw_Box(d_message_x, d_message_y, d_message_w, d_message_h, BOXSTYLE_GREEN_BORDER, true);
                Messages.Draw();

                LogicPage->Fill_Rect(d_dialog_x + 2, d_msg1_y, d_dialog_x + d_dialog_w - 4, d_msg5_y + d_txt6_h, BLACK);
        ...
        */
        ...
        case (BUTTON_JOIN | KN_BUTTON):
            ...
            parms_received = 0;
            if (Request_To_Join(namebuf, join_index, &playerlist, MPlayerHouse, MPlayerColorIdx)) {
        ...
        default:
            ...
            ...............................................................*/
            if (Messages.Get_Edit_Buf() == NULL) {
                if ((input == KN_M && joinstate == JOIN_CONFIRMED) || input == (BUTTON_SEND | KN_BUTTON)
                    || input == KN_F4) {
                    memset(txt, 0, 80);
                ...
                if (i == 2) {
                    ...
                    if (i == 3) {
                        ...
                        while (sent_so_far < message_length) {
                            ...
                            memcpy(
                                GPacket.Message.Buf, Messages.Get_Edit_Buf() + sent_so_far, COMPAT_MESSAGE_LENGTH - 5);
                            ...
                            if (joinstate == JOIN_CONFIRMED) {
                                for (i = 1; i < Players.Count(); i++) {
                                    Ipx.Send_Global_Message(
                                        &GPacket, sizeof(GlobalPacketType), 1, &(Players[i]->Address));
                                    Ipx.Service();
                                }
...
static int Request_To_Join(char* playername, int join_index, ListClass* playerlist, HousesType house, int color)
{
    ...
    */
    ...
    Ipx.Send_Global_Message(&GPacket, sizeof(GlobalPacketType), 1, &(Games[join_index]->Address));
...
Get_Join_Responses(JoinStateType* joinstate, ListClass* gamelist, ColorListClass* playerlist, int join_index)
{
    ...
    ------------------------------------------------------------------------*/
    if ((*joinstate) == JOIN_CONFIRMED && Process_Global_Packet(&GPacket, &GAddress) != 0)
        return (EV_NONE);
    ...
    ------------------------------------------------------------------------*/
        ...
        .....................................................................*/
        for (i = 0; i < Games.Count(); i++) {
            if (!strcmp(Games[i]->Name, GPacket.Name) && Games[i]->Address == GAddress) {
                /*...............................................................
...
static int Net_New_Dialog(void)
{
    ...
    TextButtonClass rejectbtn(BUTTON_REJECT,
                              TXT_REJECT,
                              TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                              //#if (GERMAN | FRENCH)
    ...
    */
    ...
    for (i = 0; i < MPlayerScenarios.Count(); i++) {
        scenariolist.Add_Item(toupper(MPlayerScenarios[i]));
    }
    ...
    while (process) {
        ...
        */
        ...
        case (BUTTON_REJECT | KN_BUTTON):
            ...
            Ipx.Send_Global_Message(&GPacket, sizeof(GlobalPacketType), 1, &(Players[index - 1]->Address));
            break;
        ...
        case (BUTTON_LEVEL | KN_BUTTON):
            ...
            Hide_Mouse();
            LogicPage->Fill_Rect(d_level_x + d_level_w + 2 * factor,
                                 d_level_y,
        ...
        default:
            ...
            ...............................................................*/
            if (Messages.Get_Edit_Buf() == NULL) {
                if (input == KN_M || input == (BUTTON_SEND | KN_BUTTON) || input == KN_F4) {
                    memset(txt, 0, 80);
