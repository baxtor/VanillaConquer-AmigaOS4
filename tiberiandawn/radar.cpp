...
void RadarClass::Radar_Cursor(int forced)
{
    ...
    */
    if (tac_cell != -1 && _last_pos == tac_cell && _last_frame == SpecialRadarFrame && !forced)
        return;
    ...
    if (_last_pos != -1) {
        ...
        **/
        ...
        Cell_XY_To_Radar_Pixel(
            last_cell_x + Lepton_To_Cell(TacLeptonWidth), last_cell_y + Lepton_To_Cell(TacLeptonHeight), x2, y2);
        x2--;
        y2--;
    ...
    */
    ...
    Cell_XY_To_Radar_Pixel(
        tac_cell_x + Lepton_To_Cell(TacLeptonWidth), tac_cell_y + Lepton_To_Cell(TacLeptonHeight), x2, y2);
    x2--;
    y2--;
...
void RadarClass::Draw_Names(void)
{
    ...
    */
    ...
    CC_Draw_Shape(RadarAnim, RADAR_ACTIVATED_FRAME, RadX, RadY + 1, WINDOW_MAIN, SHAPE_NORMAL);
    LogicPage->Fill_Rect(
        RadX + RadOffX, RadY + RadOffY, RadX + RadOffX + RadIWidth - 1, RadY + RadOffY + RadIHeight - 1, BLACK);
    ...
    for (house = HOUSE_MULTI1; house < (HOUSE_MULTI1 + MPlayerMax); house++) {
        ...
        */
        if (strlen(txt)) {
            ...
            Fancy_Text_Print(txt, RadX + RadOffX + RadIWidth - 2, y, color, BLACK, style | TPF_RIGHT);
