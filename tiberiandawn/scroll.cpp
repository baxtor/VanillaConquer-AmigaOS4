    ...
    if (!IsRubberBand /*&& !IsTentative*/) {
		...
		*/
        ...
        if (Special.IsScrollMod && y == 0
            && ((x > 3 && x < EVA_WIDTH) || (x > SeenBuff.Get_Width() - EVA_WIDTH && x < SeenBuff.Get_Width() - 3))) {
            noscroll = true;
        }
        ...
        if (!noscroll) {
			...
			*/
            if (Inertia || y == 0 || x == 0 || x == (SeenBuff.Get_Width() - 1) || y == (SeenBuff.Get_Height() - 1)) {
				...
				*/
                static int _rate[9] = {0x01C0, 0x0180, 0x0140, 0x0100, 0x00C0, 0x0080, 0x0040, 0x0020, 0x0010};
